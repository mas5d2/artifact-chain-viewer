...
const PRESETS={'marketing-site':{label:'Marketing website',na:['research_evidence_summary','current_experience_assessment','integration_context','api_integration_specs','final_content_migration_data']},'cms-replatform':{label:'CMS replatform',na:['research_evidence_summary','current_experience_assessment']},'integration-platform':{label:'Integration-heavy platform',na:['research_evidence_summary']},'application':{label:'Application / product',na:['research_evidence_summary','current_experience_assessment']},'migration-only':{label:'Migration only',na:['research_evidence_summary','current_experience_assessment','ux_flow_wireframes','component_variants_responsive_rules','desktop_mobile_creative','approved_desktop_mobile_creative']}};
const STATUS_LABEL={todo:'Not started',wip:'In progress',done:'Done',na:'Not applicable'};
let project=null;
...
function getStatus(id,feat){if(!project)return 'todo';const n=byId(id);if(n&&n.stage==='feature-level'){if(feat)return feat.status[id]||'todo';if(!project.features.length)return 'todo';const st=project.features.map(f=>f.status[id]||'todo');return st.every(s=>s==='done'||s==='na')?'done':st.some(s=>s!=='todo')?'wip':'todo'}return project.status[id]||'todo'}
// safe: every hard dep done · conditional: no dep untouched · unsafe: some dep not started
function readiness(id,feat){const cur=getStatus(id,feat);if(cur==='done'||cur==='na')return 'done';const st=((byId(id).dependsOn)||[]).filter(byId).map(d=>getStatus(d,feat));const sat=s=>s==='done'||s==='na';if(st.every(sat))return 'safe';if(st.every(s=>s!=='todo'))return 'conditional';return 'unsafe'}
// how many artifact-steps away from workable an item is (externals cost 0, artifacts 1 per layer)
function readyDist(id,feat,seen=new Set()){if(getStatus(id,feat)==='done'||seen.has(id))return 0;const s=new Set(seen);s.add(id);
const miss=((byId(id).dependsOn)||[]).filter(byId).filter(d=>getStatus(d,feat)!=='done');
return miss.length?Math.max(...miss.map(d=>byId(d).type==='external-input'?0:1+readyDist(d,feat,new Set(s)))):0}
function nbaByRole(){
...
function statusControls(n){if(!project)return '';const opts=cur=>['todo','wip','done','na'].map(s=>`<option value="${s}"${cur===s?' selected':''}>${STATUS_LABEL[s]}</option>`).join('');
...
if(!project){const pOpts=Object.entries(PRESETS).map(([k,p])=>`<option value="${k}">${esc(p.label)}</option>`).join('');el.innerHTML=`<h2>Project</h2><p class="empty">Start a project to track artifact status and get a next best action per role. Stored in this browser only.</p><div class="statusrow"><input id="pname" placeholder="Project name"><select id="ppreset"><option value="">No preset</option>${pOpts}</select><button id="pstart">Start project</button></div>`;
document.getElementById('pstart').onclick=()=>{const v=document.getElementById('pname').value.trim();if(v){const st={};const pk=document.getElementById('ppreset').value;if(pk&&PRESETS[pk])for(const id of PRESETS[pk].na)st[id]='na';project={name:v,createdAt:new Date().toISOString(),status:st,features:[]};saveProject()}};return}
...
<div class="meta">${shared.filter(n=>getStatus(n.id)==='done').length}/${shared.filter(n=>getStatus(n.id)!=='na').length} project artifacts done · ${ext.filter(n=>getStatus(n.id)==='done').length}/${ext.filter(n=>getStatus(n.id)!=='na').length} external inputs received</div>
